...
var _ = g.Describe("[Suite:openshift/oauth] LDAP IDP", func() {
	...
	g.It("should authenticate against an ldap server", func() {
		...
		_, err = oauthutil.RequestTokenForUser(tokenOpts, userName, badPass)
		o.Expect(err).To(o.HaveOccurred())

		g.By("authenticating with LDAP user")
		person1Token, err := oauthutil.RequestTokenForUser(tokenOpts, userName, goodPass)
		o.Expect(err).NotTo(o.HaveOccurred())

		// Make sure we can use the token, and it represents who we expect
		userConfig := restclient.AnonymousClientConfig(oc.AdminConfig())
		userConfig.Host = oc.AdminConfig().Host
		userConfig.CAFile = oc.AdminConfig().CAFile
		userConfig.CAData = oc.AdminConfig().CAData
